...
def _ceil_to_next_half_hour(dt: datetime.datetime) -> datetime.datetime:
    ...
    minute = dt.minute
    if minute == 0 or minute == 30: return dt
    if minute < 30: return dt.replace(minute=30)
...
def get_categories_kb(categories, booking_mode=False, cart_count=0):
    keyboard = []; row = []
    emoji_map = {'Кава': '☕️', 'До Кави': '➕', 'Декаф': '🍃', 'Кава На Альтернативному': '🥛', 'Десерти': '🍰', 'Напої': '🥤', 'Масала': '☕', 'Фреш': '🍊', 'Чай': '🫖', 'Мілк': '🥛', 'Матча': '🍵', 'Какао': '🍫'}
    fixed_order = ['Кава', 'До Кави', 'Декаф', 'Кава На Альтернативному', 'Десерти', 'Напої', 'Масала', 'Фреш', 'Чай', 'Мілк', 'Матча', 'Какао']
    def normalize_cat(cat):
        cat = str(cat)
        for emoji in emoji_map.values(): cat = cat.replace(emoji, "").strip()
