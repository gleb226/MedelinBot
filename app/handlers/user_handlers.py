...
async def booking_date_chosen(callback: CallbackQuery, state: FSMContext):
    ...
    if not getattr(time_kb, "inline_keyboard", None):
        try: await callback.message.edit_text("Немає доступного часу на обрану дату.\nОберіть іншу дату:", reply_markup=kb.get_date_kb(), parse_mode="HTML")
        except TelegramBadRequest as e:
            if "message is not modified" not in str(e): raise
        await state.set_state(BookingStates.choosing_date); return
    try: await callback.message.edit_text("🕒 <b>ОБЕРІТЬ ЧАС:</b>", reply_markup=time_kb, parse_mode="HTML")
    except TelegramBadRequest as e:
        if "message is not modified" not in str(e): raise
...
async def booking_time_chosen(callback: CallbackQuery, state: FSMContext):
    time_str = callback.data.replace("book_time_", ""); data = await state.get_data()
    try:
        ...
        closing = datetime.datetime.combine(sel_date, datetime.time(hour=0, minute=0)) + datetime.timedelta(days=1) if end_hour_raw == 24 else datetime.datetime.combine(sel_date, datetime.time(hour=end_hour_raw, minute=0))
        latest = (closing - datetime.timedelta(hours=1)).replace(minute=0, second=0, microsecond=0)
        if sel_dt > latest:
            await callback.answer("Останній доступний час — за 1 годину до закриття.", show_alert=True); return
        if sel_date == datetime.date.today() and sel_dt < datetime.datetime.now().replace(second=0, microsecond=0):
            await callback.answer("Неможливо обрати час у минулому.", show_alert=True); return
    except Exception: await callback.answer("Некоректний час.", show_alert=True); return
    ...
    await callback.message.edit_text(f"🗓️ <b>ОБРАНО:</b> {full_date}\n\n👥 <b>КІЛЬКІСТЬ ГОСТЕЙ (1-20):</b>", parse_mode="HTML")
...
async def _booking_summary_text(data: dict) -> str:
    loc_id = data.get("location_id"); loc = await location_db.get_location_by_id(loc_id)
    location_name = loc['name'] if loc else "—"
    wishes = data.get("wishes") or ""; wishes_line = wishes if wishes else "—"
    cart = data.get("cart", []) or []; cart_line = ", ".join(cart).upper() if cart else "—"
...
def _booking_summary_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="✅ ПІДТВЕРДИТИ БРОНЬ", callback_data="booking_confirm")],[InlineKeyboardButton(text="🛍 ДОДАТИ ПОЗИЦІЇ", callback_data="booking_go_menu")],[InlineKeyboardButton(text="🏠 НА ГОЛОВНУ", callback_data="back_main_menu_only")]])

async def _send_booking_summary(target, state: FSMContext):
    data = await state.get_data(); text = await _booking_summary_text(data); markup = _booking_summary_kb()
    if isinstance(target, CallbackQuery): await target.message.edit_text(text, reply_markup=markup, parse_mode="HTML")
    else: await target.answer(text, reply_markup=markup, parse_mode="HTML")
...
async def booking_wishes_entered(message: Message, state: FSMContext, bot: Bot):
    wishes_raw = (message.text or "").strip(); wishes = "" if wishes_raw.lower() in ("ні", "нет", "no", "-", "—") else wishes_raw
    await state.update_data(wishes=wishes); phone = await user_db.get_phone(message.from_user.id)
    if phone: await state.update_data(phone=phone); await _send_booking_summary(message, state); return
...
async def booking_confirm(callback: CallbackQuery, state: FSMContext, bot: Bot):
    await callback.answer(); data = await state.get_data(); cart = data.get("cart", []) or []
    if cart:
        ...
        await send_order_invoice(callback.from_user, callback.message.chat.id, state, bot); return
...
async def _create_table_booking_and_notify(user, chat_id: int, state: FSMContext, bot: Bot):
    data = await state.get_data(); loc_id = data.get("location_id"); phone = data.get("phone"); date_time = data.get("date_time"); people_count = data.get("people_count"); wishes = data.get("wishes") or ""
    loc = await location_db.get_location_by_id(loc_id)
    if not loc or not phone or not date_time or not people_count:
        await state.clear(); is_admin = await admin_db.is_admin(user.id); await bot.send_message(chat_id, "❌ <b>БРОНЮВАННЯ НЕ ВДАЛОСЯ.</b>\nСпробуйте ще раз з головного меню.", reply_markup=kb.get_main_menu(is_admin), parse_mode="HTML"); return
    booking_id = await booking_db.add_booking(user.id, user.username, user.full_name, phone, loc_id, date_time, str(people_count), wishes if wishes else "—", "СТОЛИК", "booking")
...
async def menu_item(callback: CallbackQuery, state: FSMContext):
    ...
    text = "\n".join(parts); reply_markup = kb.get_item_actions_kb(item_id)
    if image_url: await callback.message.delete(); await callback.message.answer_photo(photo=image_url, caption=text, reply_markup=reply_markup, parse_mode="HTML")
    else: await callback.message.edit_text(text, reply_markup=reply_markup, parse_mode="HTML")
...
async def menu_add_to_cart(callback: CallbackQuery, state: FSMContext):
    ...
    if not milk_type and category in ("Кава", "Кава На Альтернативному", "Масала", "Какао"):
        await callback.message.edit_reply_markup(reply_markup=kb.get_milk_kb(item_id)); return
    final_name = name
    if milk_type:
    ...
    state_data = await state.get_data(); cart = list(state_data.get("cart", [])); cart.append(final_name); await state.update_data(cart=cart); await callback.answer(f"Додано: {final_name}")
    booking_mode = bool(state_data.get("booking_mode")); categories = await menu_db.get_categories()
    if callback.message.photo: await callback.message.delete(); await callback.message.answer("🍽️ <b>МЕНЮ / ЗАМОВЛЕННЯ</b>\n\nДодано в кошик. Оберіть категорію:", reply_markup=kb.get_categories_kb(categories, booking_mode=booking_mode, cart_count=len(cart)), parse_mode="HTML")
    else: await callback.message.edit_text("🍽️ <b>МЕНЮ / ЗАМОВЛЕННЯ</b>\n\nДодано в кошик. Оберіть категорію:", reply_markup=kb.get_categories_kb(categories, booking_mode=booking_mode, cart_count=len(cart)), parse_mode="HTML")
...
async def menu_back_to_items(callback: CallbackQuery, state: FSMContext):
    ...
    if not cat: await menu_back_to_categories(callback, state); return
    items = await menu_db.get_items_by_category(cat)
    if callback.message.photo: await callback.message.delete(); await callback.message.answer(f"🍽️ <b>{cat}</b>\n\nОберіть позицію:", reply_markup=kb.get_items_kb(items, cat, cart_count=len(cart), booking_mode=booking_mode), parse_mode="HTML")
    else: await callback.message.edit_text(f"🍽️ <b>{cat}</b>\n\nОберіть позицію:", reply_markup=kb.get_items_kb(items, cat, cart_count=len(cart), booking_mode=booking_mode), parse_mode="HTML")
...
async def location_info(callback: CallbackQuery):
    ...
    await callback.answer(); await callback.message.edit_text(text, reply_markup=kb_info, parse_mode="HTML")

@user_router.callback_query(F.data == "locinfo_back")
async def location_info_back(callback: CallbackQuery):
    await callback.answer(); await callback.message.edit_text("📍 <b>НАШІ ЗАКЛАДИ</b>\n\nОберіть заклад:", reply_markup=await kb.get_locations_info_kb(), parse_mode="HTML")
...
async def beans_start(message: Message, state: FSMContext):
    ...
    beans_list = [(str(b['_id']), b['name']) for b in items]
...
async def beans_chosen(callback: CallbackQuery, state: FSMContext):
    ...
    if not bean:
        await callback.answer("Не знайдено.")
        return
    await state.update_data(bean_name=bean['name'], base_price=bean['price_250'])
    text = f"☕️ <b>{bean['name']}</b>\n\n📜 <b>Опис:</b> {bean['description']}\n🌱 <b>Сорт:</b> {bean['sort']}\n✨ <b>Смак:</b> {bean['taste']}\n🔥 <b>Обсмаження:</b> {bean['roast']}\n\n⚖️ <b>Оберіть вагу:</b>"
    reply_markup = kb.get_beans_weight_kb()
    if bean.get('image_url'): await callback.message.delete(); await callback.message.answer_photo(photo=bean['image_url'], caption=text, reply_markup=reply_markup, parse_mode="HTML")
    else: await callback.message.edit_text(text, reply_markup=reply_markup, parse_mode="HTML")
...
async def beans_back(callback: CallbackQuery, state: FSMContext):
    items = await coffee_beans_db.get_all_beans(); beans_list = [(str(b['_id']), b['name']) for b in items]
    text = "☕️ <b>КАВА В ЗЕРНАХ</b>\n\nОберіть сорт:"; reply_markup = kb.get_beans_kb(beans_list)
    if callback.message.photo: await callback.message.delete(); await callback.message.answer(text, reply_markup=reply_markup, parse_mode="HTML")
    else: await callback.message.edit_text(text, reply_markup=reply_markup, parse_mode="HTML")
    await state.set_state(CoffeeBeanStates.choosing_beans)
...
async def back_to_main_cb(callback: CallbackQuery, state: FSMContext):
    await state.clear(); is_admin = await admin_db.is_admin(callback.from_user.id)
